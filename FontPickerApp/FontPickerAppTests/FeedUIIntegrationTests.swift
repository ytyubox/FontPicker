...
final class FontUIIntegrationTests: XCTestCase {
    ...
    func test_loadingFontIndicator_isVisibleWhileLoadingFont() {
    	...
    	func test_loadFontCompletion_rendersSuccessfullyLoadedFont() {
            ...
            assertThat(sut, isRendering: image0.variants)

    		sut.simulateUserInitiatedFontReload()
    		loader.completeFontLoading(with: [image0, image1, image2, image3], at: 1)
            assertThat(sut, isRendering: [image0, image1, image2, image3].flatMap(\.variants))
    	}
    	...
    	func test_loadFontCompletion_rendersSuccessfullyLoadedEmptyFontAfterNonEmptyFont() {
            ...
            assertThat(sut, isRendering: image0.variants + image1.variants)
    	...
    	func test_feedImageView_loadsImageURLWhenVisible() {
    		let image0 = makeFont(urls: [URL(string: "http://url-0.com")!])
    		let image1 = makeFont(urls: [URL(string: "http://url-1.com")!])
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        let view0 = sut.simulateFontCellVisible(at: 0)
        let view1 = sut.simulateFontCellVisible(at: 1)
    ...
    func test_feedImageView_rendersImageLoadedFromURL() {
        ...
        sut.loadViewIfNeeded()
        loader.completeFontLoading(with: [makeFont(urls: [anyURL(), anyURL()])])
...
//        XCTAssertEqual(view1?.demoingFont, font1, "Expected image for second view once second image loading completes successfully")
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
    	...
    	func test_feedImageViewRetryButton_isVisibleOnInvalidImageData() {
    		...
    		sut.loadViewIfNeeded()
    		loader.completeFontLoading(with: [makeFont()])

    		let view = sut.simulateFontCellVisible(at: 0)
    		XCTAssertEqual(view?.isShowingRetryAction, false, "Expected no retry action while loading image")

//    		let invalidImageData = Data("invalid image data".utf8)
            imageLoader.completeImageLoading(with: .defaultFont, at: 0)
    		XCTAssertEqual(view?.isShowingRetryAction, true, "Expected retry action once image loading completes with invalid image data")
    	}

    	func test_feedImageViewRetryAction_retriesImageLoad() {
            let url = URL(string: "http://any-url.com")!
            let font = makeFont(urls: [url, url])
    		let (sut, loader,imageLoader) = makeSUT()

    		sut.loadViewIfNeeded()
    		loader.completeFontLoading(with: [font])

    		let view0 = sut.simulateFontCellVisible(at: 0)
    		let view1 = sut.simulateFontCellVisible(at: 1)
    		XCTAssertEqual(imageLoader.loadedImageURLs, [url, url], "Expected two image URL request for the two visible views")

    		imageLoader.completeImageLoadingWithError(at: 0)
    		imageLoader.completeImageLoadingWithError(at: 1)
    		XCTAssertEqual(imageLoader.loadedImageURLs, [url, url], "Expected only two image URL requests before retry action")

    		view0?.simulateRetryAction()
    		XCTAssertEqual(imageLoader.loadedImageURLs, [url, url, url], "Expected third imageURL request after first view retry action")

    		view1?.simulateRetryAction()
    		XCTAssertEqual(imageLoader.loadedImageURLs, [url, url, url, url], "Expected fourth imageURL request after second view retry action")
    	}

    	func test_feedImageView_preloadsImageURLWhenNearVisible() {
            let url1 = URL(string: "http://url-0.com")!
            let url2 = URL(string: "http://url-1.com")!
    		let font = makeFont(urls: [url1, url2])
    		let (sut, loader,imageLoader) = makeSUT()
    		...
    		XCTAssertEqual(imageLoader.loadedImageURLs, [url1], "Expected first image URL request once first image is near visible")

    		sut.simulateFontImageViewNearVisible(at: 1)
    		XCTAssertEqual(imageLoader.loadedImageURLs, [url1, url2], "Expected second image URL request once second image is near visible")
    	}
    	...
    	func test_feedImageView_cancelsImageURLPreloadingWhenNotNearVisibleAnymore() {
    		...
    		XCTAssertEqual(imageLoader.cancelledImageURLs, [url1], "Expected first cancelled image URL request once first image is not near visible anymore")

    		sut.simulateFontImageViewNotNearVisible(at: 1)
    		XCTAssertEqual(imageLoader.cancelledImageURLs, [url1, url2], "Expected second cancelled image URL request once second image is not near visible anymore")
    	}
    	...
    	func test_feedImageView_doesNotRenderLoadedImageWhenNotVisibleAnymore() {
            ...
            imageLoader.completeImageLoading(with: .defaultFont)
    ...
    func test_loadFontCompletion_dispatchesFromBackgroundToMainThread() {
    	...
    	func test_loadImageDataCompletion_dispatchesFromBackgroundToMainThread() {
    		...
    		sut.loadViewIfNeeded()
    		loader.completeFontLoading(with: [makeFont()])
    		_ = sut.simulateFontCellVisible(at: 0)
    ...
    private func makeFont(urls : [URL] = [URL(string: "http://any-url.com")!]) -> Font {
        return uniqueFont(
            variants: urls.map{
                url in
                Variant(name: "A Variant name", fileURL: url)
            }
        )
    }
